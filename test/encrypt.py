...
def decrypt_vector(private_key, parameters):
    ...
    print(f"encrypt_time: {decrypt_time:.6f}s")
    return parameters
...
if __name__=="__main__":
    ...
    for i in range(args['num_comm']):
        ...
        end_time = time.time()
        print(f"Communication round {i + 1} completed in {end_time - start_time:.6f}s")
        # 不进行计算图构建（无需反向传播）
        with torch.no_grad():
            # 满足评估的条件，用测试集进行数据评估
            if (i + 1) % args['val_freq'] == 0:
                ...
                for data, label in trainDataLoader:
                    ...
                    sum_accu += (preds == label).float().mean()
                    num += 1
                print('train_accuracy: {:.6f}'.format(sum_accu / num))
